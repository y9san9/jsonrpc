...
internal class JsonRpcResponseEngine(
    ...
    private val pending = mutableMapOf<JsonRpcResponseId, CancellableContinuation<JsonRpcResponse>>()
    private val mutex = Mutex()

    fun start() {
        backgroundScope.launch(start = UNDISPATCHED) {
            incomingEngine.responses.collect { response ->
                val continuation = mutex.withLock {
    ...
    suspend fun await(id: JsonRpcRequestId): JsonRpcResponse {
        ...
        return suspendCancellableCoroutine { continuation ->
            backgroundScope.launch(start = UNDISPATCHED) {
                mutex.withLock {
                    pending[responseId] = continuation
                }
            }
